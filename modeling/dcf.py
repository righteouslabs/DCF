    ...
    try:
        ...
        try:
            if current_price > 0 and enterprise_value > 0:
                ...
                base_fcf = (
                    enterprise_value * discount_rate
                )  # Rough estimate of current FCF
                ...
                # Calculate IRR
                irr = calculate_irr(
                    current_price * trends.get("base_shares", 1),
                    projected_cash_flows,
                    terminal_value,
                )
        ...
        return {
            ...
            "dcf_valuation": {
                **dcf_result,
                ...
                "projected_cash_flows": (
                    projected_cash_flows if "projected_cash_flows" in locals() else []
                ),
            },
            "discount_rate": discount_rate,
...
def calculate_irr(
    initial_investment: float, cash_flows: List[float], terminal_value: float
) -> float:
    ...
    all_cash_flows = (
        [-initial_investment] + cash_flows[:-1] + [cash_flows[-1] + terminal_value]
    )
    ...
    def npv_derivative(rate):
        ...
        return sum(
            -i * cf / (1 + rate) ** (i + 1) for i, cf in enumerate(all_cash_flows)
        )
