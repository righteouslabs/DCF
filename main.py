    ...
    if args.s > 0:
        ...
        if args.v is not None:
            if args.v == "eg" or "earnings_growth_rate":
                ...
                raise ValueError(
                    f'Invalid variable "{args.v}". Valid options: {valid_variables}'
                )
        else:
            raise ValueError(
        ...
        try:
            result = enhanced_DCF_with_trends(
                ...
                discount_rate=(
                    args.d if args.d != 0.1 else None
                ),  # Use dynamic WACC if default
...
def _display_results(result: dict):
    ...
    logger.info(
        f"   • Intrinsic Value per Share: ${enhanced.get('intrinsic_value', 0):.2f}"
    )
    ...
    logger.info(
        f"   • Terminal Growth Rate: {result.get('terminal_growth_rate', 0):.1%}"
    )
...
def multiple_tickers(
    ...
    for ticker in tickers:
        try:
            ...
            if years > 1:
                ...
                logger.warning(
                    f"Single-period DCF for {ticker} requires current statement data"
                )
                dcf_result = None
    ...
    logger.info(
        f"Batch DCF completed: {len(results)} successful, {len(failed_tickers)} failed"
    )
    if failed_tickers:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--p", "--period", help="years to forecast", type=int, default=5
    )
    parser.add_argument(
        "--t",
        "--ticker",
    ...
    parser.add_argument(
        "--apikey",
        help="API key for financialmodelingprep.com",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Years of historical data for trend analysis",
    )
